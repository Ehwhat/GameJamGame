...
public abstract class NavMeshEnemyBase : EnemyBase {
    ...
    {
        _destPatrolIndex = UnityEngine.Random.Range(0, _patrolPath.Length - 1);
        NextPatrolPoint();
    ...
    private void NextPatrolPoint()
    {
        ...
        _destPatrolIndex = (_destPatrolIndex + 1) % _patrolPath.Length;
    }
    ...
    protected virtual void OnDrawGizmos()
    {
        if (_debug)
        {
            ...
            if (_patrolPath.Length - 1 == _currentPatrolIndex)
            {
                Gizmos.color = Color.red;
            }
            Gizmos.DrawSphere(_patrolPath[_patrolPath.Length-1], 2);
        }
    }
