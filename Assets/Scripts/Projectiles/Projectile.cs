...
public abstract class Projectile : PooledMonoBehaviour<Projectile> {
    ...
    private bool Step()
    {
        ...
        if (didHit)
        {
            if (owner != null && hit.collider.gameObject != owner.gameObject)
            {
                position = hit.point;
    ...
    public virtual void HandleDamage(RaycastHit hit)
    {
        ...
        if (hitTarget != null && hit.collider.transform.root.gameObject != transform.root.gameObject)
        {
            hitData = new HitData(owner, this ,damage,knockback,damageType,damageInflictionWhenDirectHit, hit);
        ...
        if (useSplash)
        {
            ...
            foreach(Collider c in hitObjects)
            {
                ...
                if(explosionTarget != null)
                {
                    ...
                    hitData = new HitData(owner, this, damage*damageModifier, knockback, damageType, damageInflictionWhenSplash, hit);
